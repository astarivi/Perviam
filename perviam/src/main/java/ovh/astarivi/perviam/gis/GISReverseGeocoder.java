...
import ovh.astarivi.perviam.gis.models.GISSegment2D;
...
public class GISReverseGeocoder {
    ...
    private static final String queryBoundaries = "is_in(%f,%f);" +
            "area._[admin_level][type=boundary][boundary=administrative]->.a;" +
            "area.a[admin_level=%d]->.filtered_a;" +
            "node[place=city](area.filtered_a)->.b;" +
            ".a out;.b out;";
    ...
    public static GISBoundaries getBoundaries(@NotNull GISPoint2D location) throws PrematureStopException {
        ...
        try {
            overpassResponse = Overpass.executeQuery(
                    queryBoundaries.formatted(
                            location.latitude().value(),
                            location.longitude().value(),
                            Data.getInstance().getSettings().adminLevelCity
                    ),
        ...
        TreeSet<BoundaryElement> boundaries = new TreeSet<>();

        boolean isCity = false;

        for (BoundaryElement element : overpassResponse.elements) {
        ...
        return new GISBoundaries(boundaries, isCity);
    }
}
